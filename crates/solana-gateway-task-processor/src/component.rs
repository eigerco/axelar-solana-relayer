...
async fn execute_task(
    ...
    let gateway_message_payload_pda =
        message_payload::upload(keypair, metadata.gateway_root_pda, &message, &payload).await?;
    ...
    message_payload::close(keypair, metadata.gateway_root_pda, &message).await?;
    Ok(())
}
